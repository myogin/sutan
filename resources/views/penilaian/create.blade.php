								...
								<div class="row">
									<div class="col-lg-8">
										...
										<div class="card card-custom example example-compact">
											...
											<!--begin::Form-->
											<form class="form" id="kt_form_1" action="{{route('penilaian.store')}}" method="POST" enctype="multipart/form-data">
                                                @csrf
												<div class="card-body">
                                                    <!--begin::ALLERT-->
                                                    @if(session('status'))
                                                    ...
                                                    <div class="form-group row">
                                                        <label class="col-form-label col-lg-3 col-sm-12 text-right">Siswa *</label>
                                                        <div class="col-lg-4 col-md-9 col-sm-12">
                                                            <select class="form-control select2" id="siswa" name="siswa">
                                                    ...
                                                    @foreach ($kriteria as $key => $sk)
                                                    <div class="form-group row">
                                                        <label class="col-form-label col-lg-3 col-sm-12 text-right">{{$sk->kriteria}}</label>
                                                        <input type="hidden" name="kriteria[]" value="{{$sk->id}}">
                                                        <div class="col-lg-4 col-md-9 col-sm-12">
                                                            <select class="form-control select2" id="sbk{{$sk->id}}" name="subkriteria[]" required>
                                                                <option  label="Label"></option>
                                                                @foreach ($sk->subkriteria as $key => $skr)
                                                                <option value="{{$skr->id}}">{{$skr->sub_kriteria}}</option>
                                                                @endforeach --}}
                                                            </select>
                                                            <span class="form-text text-muted">Select an option</span>
                                                        </div>
...
var KTFormControls = function () {
	...
	var _initDemo1 = function () {
        ...
        $('#siswa').on('change', function(){
            // Revalidate field
            validator.revalidateField('siswa');
        });

        var krta = [
@foreach ($kriteria as $sk)
    [ "{{ $sk->id }}"],
@endforeach
];
for (i = 0; i < krta.length; i++) {
    $('#sbk'+krta[i]).select2({
        placeholder: "Select a state",
    });
}
